...
shape its tool emits and assert on the parsed Signal.
"""
...
def test_parse_junit_all_passing(tmp_path: Path) -> None:
    ...
    detail, status = _parse_junit(path, fallback="")
...
def test_parse_junit_missing_file_falls_back(tmp_path: Path) -> None:
    detail, status = _parse_junit(str(tmp_path / "missing.xml"), fallback="raw output here")
...
def test_parse_jest_success(tmp_path: Path) -> None:
    ...
    detail, status = _parse_jest(path, fallback="")
    assert status is GateStatus.PASS
...
def test_parse_jest_failure(tmp_path: Path) -> None:
    path = _write(
        tmp_path, "r.json",
        json.dumps({"success": False, "numPassedTests": 1, "numFailedTests": 1, "numTotalTests": 2}),
    )
    detail, status = _parse_jest(path, fallback="")
    assert status is GateStatus.FAIL
...
def test_parse_go_test_pass() -> None:
    detail, status = _parse_go_test(_GO_STREAM_PASS, fallback="")
    assert status is GateStatus.PASS
    assert "1 passed, 0 failed" in detail


def test_parse_go_test_fail() -> None:
    detail, status = _parse_go_test(_GO_STREAM_FAIL, fallback="")
    assert status is GateStatus.FAIL
    assert "pkg.TestAdd" in detail


def test_parse_go_test_no_events_falls_back() -> None:
    detail, status = _parse_go_test("not json at all", fallback="build failed: syntax error")
    assert status is None
...
def test_parse_tsc_clean_run() -> None:
    detail, status = _parse_tsc(_proc(stdout="", returncode=0))
    assert status is GateStatus.PASS
...
def test_parse_tsc_with_errors() -> None:
    ...
    detail, status = _parse_tsc(_proc(stdout=stdout, returncode=1))
    assert status is GateStatus.FAIL
...
def test_parse_mypy_clean_run() -> None:
    detail, status = _parse_mypy(_proc(stdout="", returncode=0))
    assert status is GateStatus.PASS
...
def test_parse_mypy_with_errors() -> None:
    ...
    detail, status = _parse_mypy(_proc(stdout=line, returncode=1))
    assert status is GateStatus.FAIL
    assert "1 error(s)" in detail
...
def test_parse_eslint_clean_run() -> None:
    ...
    detail, status = _parse_eslint(_proc(stdout=payload, returncode=0))
    assert status is GateStatus.PASS
    assert "0 error(s)" in detail
...
def test_parse_eslint_with_errors() -> None:
    ...
    detail, status = _parse_eslint(_proc(stdout=payload, returncode=1))
    assert status is GateStatus.FAIL
    assert "1 error(s)" in detail
...
def test_parse_ruff_clean_run() -> None:
    detail, status = _parse_ruff(_proc(stdout="[]", returncode=0))
    assert status is GateStatus.PASS
...
def test_parse_ruff_with_violations() -> None:
    ...
    detail, status = _parse_ruff(_proc(stdout=payload, returncode=1))
    assert status is GateStatus.FAIL
